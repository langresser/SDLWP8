...
void SDL_BlitRects(SDL_Surface * screen, int numrects, SDL_Rect * rects)
{
	int i;
...
void SDL_BlitToScreen(SDL_Surface * surface, int numrects, SDL_Rect * src, SDL_Rect* dst)
{
	int i;
	for (i = 0; i < numrects; ++i) {
		SDL_BlitSurface(surface, &src[i], SDL_VideoSurface,
			&dst[i]);
		//SDL_SoftStretch(surface, NULL, SDL_VideoSurface, dst);
	}
}

void SDL_FlipRects(int numrects, SDL_Rect * rects)
{
	int i;
	if (SDL_VideoViewport.x || SDL_VideoViewport.y) {
		SDL_Rect *stackrects = SDL_stack_alloc(SDL_Rect, numrects);
