...
GLES2_UpdateClipRect(SDL_Renderer * renderer)
{
    ...
    if (!SDL_RectEmpty(rect)) {
        ...
        glScissor(rect->x, renderer->viewport.h - rect->y - rect->h, rect->w, rect->h);
    } else {
...
extern FILE* open_file(const char* file_name, const char* read_mode);
unsigned char* getShaderData(const char* pszFileName, unsigned long * pSize)
{
	...
	*pSize = 0;
	do
	{
		FILE *fp = open_file(pszFileName, "rb");
		if (!fp) {break;}
...
#ifdef ANGLE
		pBuffer = getShaderData(instance->file_name, &ssize);
		glShaderBinary(1, &entry->id, 1, pBuffer, ssize);
		free(pBuffer);
#endif
    }
    else
